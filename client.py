...
class ShamiraClient:
    def __init__(self, node_host, current_node, get_session=None, keypair_file_path="key_pair.pem", private_key=None, public_key=None ):
        ...
        self.keypair_file_path = keypair_file_path or "key_pair.pem"  # Replace with the actual file path
        
        if not(private_key and public_key):
        
            if os.path.isfile(self.keypair_file_path):
                self.private_key, self.public_key = utils.load_key_pair(self.keypair_file_path)
                logger.info("Loaded existing key pair from '%s'.", self.keypair_file_path)
            else:
    ...
    def register_node(self, hostname, alias, description=""):
        logger.info("Registering node %s with hostname: %s", self.current_node.hostname, hostname)
    ...
    def login_and_get_token(self):
        logger.info("Logging in as node %s to node with hostname: %s", self.current_node.hostname, self.node_host)
