...
from server import Base as OriginalBase
from server import (  # Import your FastAPI application
    Job,
    Node,
    ShamiraTask,
    Step,
...
def get_engine(node_host=None):
    return create_engine(
        f"sqlite:///./tests/data/{node_host}.db",
...
def get_client_session(node_host):
    ...
    def override_get_db():
        ...
        print(
            "Creating new test database session for node host:", f"test_node_{node_id}"
        )
...
def reconstruct_sharded_value(db, variable_name, public_key_to_node_index):
    ...
    reconstructed_point = lagrange_interpolation(pol, FiniteFieldElement(0, N))
...
class TestNodePropagation(unittest.TestCase):
    def setUp(self):
        ...
        for i in range(NUM_NODES):
            ...
            OriginalBase.metadata.create_all(
                bind=get_engine(node_host=f"test_node_{i}")
            )
    ...
    def test_propagate_nodes(self):
        ...
        for i in range(NUM_NODES):
            ...
            for node in nodes:
                nodes_stored = {}
                print(
                    f"Node in test_node_{i} DB: {node.hostname}, {node.alias},"
                    f" {node.description}, {node.public_key}"
                )
    ...
    def test_propagate_jobs(self):
        ...
        new_job = Job(
            name="Test Job", status="waiting", hash_id=f"job-{uuid.uuid4().hex}"
        )
        db.add(new_job)
    ...
    def test_execute_jobs_with_var_reuse_add(self):
        ...
        self.assertEqual(
            len(steps),
            NUM_NODES,
            "Number of steps for variable w should be equal to number of nodes",
        )

        reconstructed_point_value_x = reconstruct_sharded_value(
            db, "x", public_key_to_node_index
        )
        reconstructed_point_value_y = reconstruct_sharded_value(
            db, "y", public_key_to_node_index
        )
        reconstructed_point_value_z = reconstruct_sharded_value(
            db, "z", public_key_to_node_index
        )
        reconstructed_point_value_w = reconstruct_sharded_value(
            db, "w", public_key_to_node_index
        )

        self.assertEqual(reconstructed_point_value_x, 5 + 4)
        self.assertEqual(reconstructed_point_value_y, 6 + 4)
        self.assertEqual(reconstructed_point_value_z, 5 + 6)
        self.assertEqual(reconstructed_point_value_w, (5 + 4) + (6 + 4) + (5 + 6))
    ...
    def test_shards_to_private_var(self):
        ...
        self.assertEqual(
            len(steps), 1, "Number of steps for variable z should be equal to 1"
        )
        print(steps[0].output_value)
        encrypted_value = EncryptedFiniteFieldElement(None, None).deserialize(
            steps[0].output_value
        )
    ...
    def test_private_computation_to_shards(self):
        ...
        a_shard_steps = (
            db.query(Step)
            .filter(Step.variable_name == "a_sharded", Step.variable_type == "shard")
            .all()
        )
        ...
        b_shard_steps = (
            db.query(Step)
            .filter(Step.variable_name == "b_sharded", Step.variable_type == "shard")
            .all()
        )
        ...
        self.assertEqual(
            len(a_shard_steps),
            NUM_NODES,
            f"Should have {NUM_NODES} sharding steps for a_sharded",
        )
        self.assertEqual(
            len(b_shard_steps),
            NUM_NODES,
            f"Should have {NUM_NODES} sharding steps for b_sharded",
        )
    ...
    def test_execute_jobs_private_vars(self):
        ...
        steps = (
            db.query(Step)
            .filter(Step.variable_name == "y", Step.variable_type == "private_variable")
            .all()
        )

        self.assertTrue(
            len(steps) >= 1, "Expected at least one step for private variable y"
        )
        ...
        for step_item in steps_sorted:
            ...
            output_x_encrypted = EncryptedFiniteFieldElement(None, None).deserialize(
                step_item.output_value
            )
            output_x = output_x_encrypted.decrypt(private_key)
            output_values.append(output_x)

        self.assertTrue(
            all(val == output_values[0] for val in output_values),
            "All private variable outputs should match",
        )
    ...
    def test_add_priv_and_shard_1(self):
    ...
    def test_test_affine_transform(self):
        ...
        y = x.compute_affine(
            a,
            r,
            source_node_public_key=node_bob.public_key,
            dest_node_public_key=node_alice.public_key,
            keypair_file_path=f"tests/data/test_node_0_keypair.pem",
        )
        ...
        step = (
            ...
            .filter(
                ...
                == get_current_node(get_db(f"test_node_0")).public_key,
            )
            .first()
        )
        ...
        # print("Private key:")
        # print(private_key)
        output_x = EncryptedFiniteFieldElement(None, None).deserialize(
            output_x_encrypted
        )
    ...
    def test_affine_transform_with_computation(self):
        ...
        step = (
            db.query(Step)
            .filter(
                Step.variable_name == "(Enc(x)*a + r) + (b)",
                Step.variable_type == "private_variable",
                Step.dest_node_public_key
                == get_current_node(get_db(f"test_node_0")).public_key,
            )
        ...
        self.assertEqual(
            step.status, StepStatusEnum.completed, "Y Step should be completed"
        )
    ...
    def test_beaver_triple_gen(self):
        ...
        ShamiraTask(
            db=db,
            name="(Aa_sharded) + (Ab_sharded)",
            value=a,
            variable_type=VariableTypeEnum.private_variable,
        )
        ShamiraTask(
            db=db,
            name="(Ba_sharded) + (Bb_sharded)",
            value=b,
            variable_type=VariableTypeEnum.private_variable,
        )
        ShamiraTask(
            db=db,
            name="(Ca_sharded) + (Cb_sharded)",
            value=c,
            variable_type=VariableTypeEnum.private_variable,
        )
        ...
        Aa_step = (
            db.query(Step)
            .filter(
                Step.variable_name == "Aa", Step.variable_type == "private_variable"
            )
            .first()
        )

        output_encrypted = EncryptedFiniteFieldElement(None, None).deserialize(
            Aa_step.output_value
        )
        public_key, private_key = generate_keypair(
            keypair_file_path=f"tests/data/test_node_0_keypair.pem"
        )
        ...
        Ab_step = (
            db.query(Step)
            .filter(
                Step.variable_name == "Ab", Step.variable_type == "private_variable"
            )
            .first()
        )

        output_encrypted = EncryptedFiniteFieldElement(None, None).deserialize(
            Ab_step.output_value
        )
        public_key, private_key = generate_keypair(
            keypair_file_path=f"tests/data/test_node_1_keypair.pem"
        )
        ...
        Ba_step = (
            db.query(Step)
            .filter(
                Step.variable_name == "Ba", Step.variable_type == "private_variable"
            )
            .first()
        )

        output_encrypted = EncryptedFiniteFieldElement(None, None).deserialize(
            Ba_step.output_value
        )
        public_key, private_key = generate_keypair(
            keypair_file_path=f"tests/data/test_node_0_keypair.pem"
        )
        ...
        Bb_step = (
            db.query(Step)
            .filter(
                Step.variable_name == "Bb", Step.variable_type == "private_variable"
            )
            .first()
        )

        output_encrypted = EncryptedFiniteFieldElement(None, None).deserialize(
            Bb_step.output_value
        )
        public_key, private_key = generate_keypair(
            keypair_file_path=f"tests/data/test_node_1_keypair.pem"
        )
        ...
        for var_name in [
            "(Aa_sharded) + (Ab_sharded)",
            "(Ba_sharded) + (Bb_sharded)",
            "(Ca_sharded) + (Cb_sharded)",
        ]:

            steps = (
                db.query(Step)
                .filter(Step.variable_name == var_name, Step.variable_type == "shard")
                .all()
            )
            ...
            self.assertEqual(
                len(steps),
                NUM_NODES,
                f"Number of steps for variable {var_name} should be equal to number of"
                " nodes",
            )

            reconstructed_point_value = reconstruct_sharded_value(
                db, var_name, public_key_to_node_index
            )

            beaver_tripple_values[var_name] = reconstructed_point_value

        self.assertEqual(
            beaver_tripple_values["(Ba_sharded) + (Bb_sharded)"], ba_output + bb_output
        )
        self.assertEqual(
            beaver_tripple_values["(Aa_sharded) + (Ab_sharded)"], aa_output + ab_output
        )

        self.assertEqual(
            (ba_output + bb_output) * (aa_output + ab_output),
            beaver_tripple_values["(Ca_sharded) + (Cb_sharded)"],
        )

        self.assertEqual(
            beaver_tripple_values["(Aa_sharded) + (Ab_sharded)"]
            * beaver_tripple_values["(Ba_sharded) + (Bb_sharded)"],
            beaver_tripple_values["(Ca_sharded) + (Cb_sharded)"],
        )

    def test_beaver_triple_with_operation(self):
        ...
        db_job_data = d_private.job.model_dump(exclude={"steps"})
        if not db_job_data.get("hash_id"):
            db_job_data["hash_id"] = f"job-{uuid.uuid4().hex}"
        db_job = Job(**db_job_data)
        db.add(db_job)
