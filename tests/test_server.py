...
from utils import generate_key_pair, sign_message
...
class TestNodeMethods(unittest.TestCase):
    def setUp(self):
        ...
        self.token = login_and_get_token(
            self.client, self.priv_key, self.pub_key_serialized
        )
    ...
    def test_list_nodes(self):
        ...
        self.assertDictEqual(
            response.json()[0],
            {
                ...
                "public_key": self.pub_key_serialized,
            },
        )
...
class TestAuthMethods(unittest.TestCase):
    ...
    def test_register_and_login(self):
        ...
        response = self.client.get(
            "/nodes", headers={"Authorization": f"Bearer {token}"}
        )
        self.assertEqual(response.status_code, 200)
...
class TesJobMethods(unittest.TestCase):
    ...
    def test_add_job(self):
        ...
        job_data = {"name": "testjob", "status": "waiting", "steps": []}
