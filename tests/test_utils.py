...
class TestUtils(unittest.TestCase):
    def test_polynomial_evaluation_and_interpolation(self):
        ...
        reconstructed_secret = lagrange_interpolation(
            selected_shares, FiniteFieldElement(0, N)
        )
    ...
    def test_get_shares_and_encryption(self):
        ...
        for share in shares:
            ...
            json_data = {"x": x_encrypted.serialize(), "y": y_encrypted.serialize()}

            x_decrypted = (
                EncryptedFiniteFieldElement(None, None)
                .deserialize(json_data["x"])
                .decrypt(private_key)
            )
            y_decrypted = (
                EncryptedFiniteFieldElement(None, None)
                .deserialize(json_data["y"])
                .decrypt(private_key)
            )
    ...
    def test_encrypt_affine_transform(self):
        ...
        self.assertEqual(
            affine_transform_decrypted.value,
            (x.value * coef.value + intercept.value) % N,
        )
