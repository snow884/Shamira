...
class FiniteFieldElement:
    ...
    def __pow__(self, exponent):
        return FiniteFieldElement(
            pow(self.value, exponent.value, self.prime), self.prime
        )
...
class EncryptedFiniteFieldElement:
    ...
    def __add__(self, other):
        ...
        if not isinstance(other, EncryptedFiniteFieldElement):
            raise TypeError(
                "Can only add EncryptedFiniteFieldElement to"
        ...
        if self.ephemeral_public_key != other.ephemeral_public_key:
            raise ValueError(
                "Cannot add encrypted values with different ephemeral keys"
            )
        ...
        return EncryptedFiniteFieldElement(
            adjusted_sum, self.ephemeral_public_key, self.shared_secret
        )
    ...
    def __mul__(self, scalar):
        ...
        if isinstance(scalar, EncryptedFiniteFieldElement):
            ...
            if self.ephemeral_public_key == scalar.ephemeral_public_key:
                ...
                return EncryptedFiniteFieldElement(
                    sum_encrypted, self.ephemeral_public_key, self.shared_secret
                )
        ...
        if isinstance(scalar, int):
            ...
            raise TypeError(
                "Scalar must be FiniteFieldElement, int, or"
                " EncryptedFiniteFieldElement, not type {}, value {}".format(
                    type(scalar), scalar
                )
            )
        ...
        return EncryptedFiniteFieldElement(
            adjusted_scaled, self.ephemeral_public_key, self.shared_secret
        )
    ...
    def __pow__(self, exponent):
        ...
        if isinstance(exponent, EncryptedFiniteFieldElement):
            ...
            raise TypeError(
                "Exponent must be FiniteFieldElement, int, or"
                " EncryptedFiniteFieldElement not type {}, value {}".format(
        ...
        if self.shared_secret is not None:
            adjustment = (
                exponent_to_use - FiniteFieldElement(1, N)
            ) * self.shared_secret
        ...
        return EncryptedFiniteFieldElement(
            adjusted_powered, self.ephemeral_public_key, self.shared_secret
        )
    ...
    def decrypt(self, private_key):
        ...
        return decrypt_message(
            private_key, self.ephemeral_public_key, self.encrypted_value
        )
    ...
    def deserialize(self, data):
        ...
        self.ephemeral_public_key = Point(None, None).deserialize(
...
class Point:
    def __init__(self, x, y):
        ...
        self.x = (
            FiniteFieldElement(x, P) if not isinstance(x, FiniteFieldElement) else x
        )
        self.y = (
            FiniteFieldElement(y, P) if not isinstance(y, FiniteFieldElement) else y
        )
...
def sign_message(private_key, message):
    ...
    r = P_k.x.value % N
    if r == 0:
        return sign_message(private_key, message)  # Recalculate if r is 0

    k_inv = modular_inverse(k, N)
    s = (k_inv * (z + r * private_key)) % N
    if s == 0:
        return sign_message(private_key, message)  # Recalculate if s is 0
...
def encrypt_message(
    ...
    if ephemeral_public_key is not None and shared_secret_n is not None:
        ...
        ephemeral_public_key = Point(G_X, G_Y).scalar_multiplication(
            ephemeral_private.value
        )
    ...
    if return_class:
        return EncryptedFiniteFieldElement(
            encrypted_element, ephemeral_public_key, shared_secret_n
        )
    else:
...
def polynomial_evaluation(
    coef: list[FiniteFieldElement], x: FiniteFieldElement, c=FiniteFieldElement(0, N)
):
    result = c
...
def get_shares(S, n_shares, k_threshold, x_coords=None):
    ...
    for i in range(n_shares):
        x = (
            FiniteFieldElement(x_coords[i], N)
            if x_coords
            else FiniteFieldElement(i + 1, N)
        )
